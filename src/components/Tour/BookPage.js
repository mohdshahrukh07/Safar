...
export default function BookPage() {
    ...
    const [ladingSpinner, setLoadingSpinner] = useState(false);
    ...
    const submit = async (e) => {
        try {
            ...
            if (response.status) {
                setDialogTitle("Booking Completed!")
                setDialogMsg("Your booking has been successfully confirmed.")
                setShowSuccess(true);
                setLoadingSpinner(false);
                console.log(response);
            } else {
                setDialogTitle("Failed!")
                setDialogMsg("Your booking has been failed. Please try again")
                setShowSuccess(true);
                setLoadingSpinner(false);
            }
        } catch (error) {
    ...
    return (
        <div>
            ...
            <ConfirmDialog
                show={showDialog}
                message={"Are you want to book this package"}
                onConfirm={() => {
                    setShowDialog(false);
